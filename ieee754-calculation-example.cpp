...
float ieee_754(uint32_t const data) {
    ...
    uint32_t mantissa;  // 24-bit mantissa: 23 bits from input, 1 bit added
    uint8_t exp;        // 8 bit unsigned exponent
    uint8_t sign;       // 1 bit of sign

    /* Extract bit ranges */
    mantissa = data & ((1 << mantissa_width) - 1);
    exp = (data >> mantissa_width) & ((1 << exp_width) - 1);
    sign = data >> (width - 1);

    /* Set implied mantissa bit if exponent is nonzero */
    if (exp != 0)
        mantissa |= 1 << mantissa_width;

    /* Calculate value using floating point operations */
    value = (float) mantissa
        * powf(2., (float) ((int16_t)exp - bias - mantissa_width))
        * (sign ? -1. : 1.);
